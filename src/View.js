...
class View extends EventEmitter {
    constructor() {
        ...
        this.loginBtn.addEventListener('click', this.loginSubmit.bind(this));
    ...
    getPhoneNumber() {
        const number = document.getElementById('login-input').value

        if (number.length === 12 && +number == number) {
            return `+${number}`;
        }

        if (number.length === 13 && number[0] === '+' && +number.slice(1, number.length - 1) == number.slice(1, number.length - 1)) {
            return number;
        }

        return alert('Неправильно введен телефон');
    }
